...
export default function HeroDesktop() {
  ...
  return (
    <section
      id="hero"
      className="relative bg-[#FAFAFA] overflow-hidden selection:bg-purple-200 selection:text-purple-900"
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-16 lg:py-24 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-[45fr_55fr] gap-12 lg:gap-8 items-center">
          <div className="flex flex-col gap-6">
            ...
            <p className="text-lg font-semibold text-slate-800 max-w-md">
            ...
            <div className="flex items-center gap-4 mt-6 p-4 rounded-2xl bg-white/40 backdrop-blur-sm border border-white/60 shadow-sm w-fit">
              <div className="flex -space-x-3">
                {[11, 12, 13, 14].map((i) => (
                  <div
                    key={i}
                    className="w-10 h-10 rounded-full border-[2.5px] border-white overflow-hidden bg-gradient-to-br from-purple-100 to-fuchsia-100 shadow-sm shrink-0"
                  >
                    <img
          ...
          <div className="relative flex items-center justify-center h-[560px]">
            <div
              ...
              style={{
                background:
                  "radial-gradient(circle at center, rgba(107,33,168,0.06) 0%, rgba(107,33,168,0.02) 45%, transparent 70%)",
              }}
            />
            ...
            >
              ...
              <circle
                ...
                r="120"
                fill="none"
                className="stroke-purple-800/15"
                strokeWidth="1"
              />
            </svg>
            ...
            {cardAngles.map((angle, i) => (
              <div
                key={`line-${i}`}
                className="absolute pointer-events-none bg-gradient-to-r from-purple-800/0 via-purple-800/20 to-purple-800/0 h-[1.5px] origin-left"
                style={{
                  left: "50%",
                  top: "50%",
            ...
            {cardAngles.map((angle, i) => {
              ...
              return (
                <div
                  key={`dot-${i}`}
                  className={`absolute w-3 h-3 rounded-full pointer-events-none transition-colors duration-300 ${
                    isActive ? "bg-purple-600 shadow-[0_0_12px_rgba(147,51,234,0.6)]" : "bg-purple-800/30"
                  }`}
                  style={{
            ...
            <div
              className="absolute z-10 flex items-center justify-center rounded-full border-[6px] border-white/80 backdrop-blur-sm shadow-2xl shadow-purple-900/30 w-40 h-40"
              style={{
                left: "50%",
                top: "50%",
                transform: "translate(-50%, -50%)",
                background:
                  "linear-gradient(135deg, #7C3AED 0%, #6B21A8 50%, #4C1D95 100%)",
              }}
            >
              <span className="text-transparent bg-clip-text bg-gradient-to-b from-white to-purple-200 font-extrabold text-4xl tracking-tighter drop-shadow-sm">
                N4RE
              </span>
            </div>
            ...
            {ECOSYSTEM_CATEGORIES.map((cat, i) => {
              ...
              return (
                <button
                  ...
                  className={`absolute z-20 flex flex-col items-center gap-2 rounded-2xl backdrop-blur-md text-center transition-all duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] outline-none focus-visible:ring-2 focus-visible:ring-purple-600 ${
                    isActive
                      ? "bg-white/95 border border-purple-400 shadow-2xl shadow-purple-900/20 scale-[1.15] z-30"
                      : "bg-white/70 border border-white/50 shadow-lg shadow-slate-200/50 hover:bg-white/90 hover:scale-105 hover:border-purple-200 hover:shadow-xl hover:shadow-purple-800/10 cursor-pointer"
                  }`}
                  style={{
                    left: `calc(50% + ${pos.x}px)`,
                    top: `calc(50% + ${pos.y}px)`,
                    ...
                    minWidth: isLg ? "150px" : isSm ? "120px" : "135px",
                  }}
                >
                  <div
                    className={`flex items-center justify-center rounded-full transition-colors duration-300 ${
                      isActive ? "bg-purple-100 border-purple-200" : "bg-purple-50/80 border-purple-900/5"
                    } border`}
                    style={{
                      width: isLg ? "46px" : "40px",
                      height: isLg ? "46px" : "40px",
                    }}
                  >
                  ...
                  <div className="flex flex-col gap-1">
                    <span
                      className={`font-bold leading-tight whitespace-pre-line transition-colors duration-300 ${
                        isActive ? "text-purple-900" : "text-slate-800"
                      }`}
                      style={{ fontSize: isLg ? "13px" : "12px" }}
                    >
                      {cat.label}
                    </span>
                    ...
                    >
                      <div className="min-h-0">
                        <span className="block text-[11px] text-slate-500 font-medium leading-snug px-1 pb-1">
                          {cat.description || "Explore this area of the ecosystem."}
                        </span>
                      </div>
                    </div>
                  </div>
