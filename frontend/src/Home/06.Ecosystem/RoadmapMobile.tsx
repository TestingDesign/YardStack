          ...
          <div className="grid grid-cols-4 gap-y-6 gap-x-2">
            {ECOSYSTEM_MEMBERS.map((member, i) => (
              <div key={i} className="flex flex-col items-center gap-1.5 ys-fade-in" style={{ animationDelay: `${i * 30}ms` }}>
                <div className="w-10 h-10 rounded-[2px] bg-white/10 flex items-center justify-center border border-white/5">
        ...
        <div className="px-4">
          ...
          <div className="flex flex-col gap-3 mt-6">
            {ROADMAP_ITEMS.map((item, i) => (
              <div
                key={i}
                className="flex items-center gap-3 p-4 rounded-[4px] bg-white border border-gray-100 shadow-[0_2px_8px_rgba(0,0,0,0.04)] ys-fade-in-up relative overflow-hidden"
                style={{ animationDelay: `${i * 60}ms` }}
              >
                <div className="w-12 h-12 rounded-[2px] flex items-center justify-center shrink-0" style={{ backgroundColor: item.bgColor }}>
