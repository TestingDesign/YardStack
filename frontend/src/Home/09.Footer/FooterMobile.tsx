...
export default function FooterMobile() {
  return (
    <footer 
      id="footer" 
      className="relative bg-[linear-gradient(175deg,#2a1550_0%,#1A1A2E_30%,#16213E_60%,#1A1A2E_80%,#16213E_100%)] py-8 border-t border-white/10 selection:bg-fuchsia-500/30 selection:text-white"
    >
      <div className="px-4 flex flex-col items-center gap-6">
        ...
        <div className="flex flex-col items-center gap-3">
          <img 
            src={LogoPng} 
            alt="N4RE Logo" 
            className="h-10 w-auto object-contain brightness-0 invert opacity-95 transition-opacity duration-300" 
          />
        ...
        <nav className="flex flex-wrap items-center justify-center gap-x-5 gap-y-3">
          {FOOTER_LINKS.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-[12px] font-bold text-white/65 hover:text-white transition-colors duration-300 outline-none"
            >
              {link.label}
            </a>
          ))}
