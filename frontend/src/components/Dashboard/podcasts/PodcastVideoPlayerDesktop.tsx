...
export default function PodcastVideoPlayerDesktop({
  ...
  return (
    ...
    >
      <div
        ref={containerRef}
        className={`relative w-full ${isOverlaid ? (inline ? 'aspect-video' : 'h-full max-w-none rounded-none') : 'flex flex-col'} overflow-hidden bg-[#05030a] ${inline ? '' : 'border border-white/10 max-w-[1200px] mx-auto sm:mx-6 rounded-2xl shadow-[0_20px_80px_rgba(66,32,130,0.2)]'} group`}
      >
        <div 
        ...
        >
          ...
          {centerPlayBtnDesktop(inline ? 60 : 80)}
        </div>
        ...
        <div
          className={`
            ${isOverlaid ? 'absolute bottom-0 inset-x-0 z-30 flex flex-col justify-end px-4 sm:px-8 pb-4 sm:pb-8 pt-16 sm:pt-32 bg-gradient-to-t from-black/80 to-transparent' : 'relative w-full bg-[#05030a] p-4 sm:p-5 z-30 flex-shrink-0 border-t border-white/5'}
        ...
        >
          <div className="flex items-end gap-5 mb-6">
            <div className="w-16 h-16 border-2 rounded-full shrink-0 bg-gradient-to-br from-[#422082] to-[#6a5fc1] flex items-center justify-center shadow-[0_8px_24px_rgba(66,32,130,0.6)] border-white/20">
            ...
            <div className="min-w-0 flex-1 drop-shadow-lg">
              <h2 className="text-[24px] mb-2 font-bold text-white leading-tight line-clamp-1">
                {episode.title}
              </h2>
