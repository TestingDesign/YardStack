...
import { parseDuration, ProgressBar } from '../podcasts/PodcastVideoPlayerShared'

export default function InlinePulsePlayer({ duration }: { duration: string }) {
  const [progress, setProgress] = useState(0)
  const totalDuration = parseDuration(duration) || 120
  ...
  return (
    <div className="absolute inset-0 bg-black z-20 border border-white/10 box-border pointer-events-auto rounded-[inherit] overflow-hidden">
      <video
        ref={videoRef}
      ...
      <button 
        className="absolute top-0 right-0 p-2 z-20 outline-none tap-highlight-transparent cursor-pointer"
