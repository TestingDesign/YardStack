...
function StatCard({
  ...
  return (
    ...
    >
      <div className={`w-7 h-7 rounded-full ${color} ${bg} filter brightness-95 flex items-center justify-center shrink-0`}>
        {icon}
      </div>
            ...
            >
              ...
              <div className="absolute bottom-3 left-3 right-3 z-20">
                ...
                <p className="text-white/90 text-[11px] font-medium flex items-center gap-1">
                  <span className="truncate">{video.author}</span>
                  {video.verified && <VerifiedIcon sx={{ fontSize: 10 }} className="text-blue-400" />}
                </p>
              </div>
            </div>
          )
                  ...
                  {hasMore && (
                    <div className="mt-8 flex items-center justify-center opacity-0 animate-swipe-up" style={{ animationDelay: '300ms' }}>
                      <button
                        ...
                        disabled={isLoading}
                        className="group flex items-center justify-center gap-2 px-6 py-2.5 rounded-md bg-purple-50 text-[13px] font-medium text-purple-700 hover:bg-purple-600 hover:text-white transition-all duration-300 cursor-pointer active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed border-none"
                      >
